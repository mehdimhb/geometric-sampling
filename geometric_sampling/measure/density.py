...
class Density:
    ...
    def _max_legacy_score(self, labels: NDArray, centroids: NDArray) -> float:
        ...
        return np.linalg.norm(density - shifted_density)
    ...
    def _density(self, shifted_coords: NDArray) -> float:
        ...
        legacy = np.linalg.norm(density - shifted_density)
        measure = [
            spread,
            var_scaled,
    ...
    def _score_sample(
        ...
        shifted_coords = self._generate_shifted_coords(shifts, labels)
        density, shifted_density, scores = self._density(shifted_coords)
        legacy_score = scores[3] / self._max_legacy_score(labels, centroids)
        scores[3] = 1 - (1 - legacy_score) ** sample.shape[0]
        return density, shifted_density, scores

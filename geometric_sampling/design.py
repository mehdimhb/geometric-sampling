    ...
    def __init__(
        ...
        if inclusions is not None:
            if perm is not None:
                ...
                self.perm = np.arange(len(inclusions))
            
            # Build the map from permuted index back to original index
            # perm: index in permuted array -> index in original array
            # inverse_perm: index in original array -> index in permuted array
            self.inverse_perm = np.empty(len(self.perm), dtype=int)
        ...
        if inclusions is not None:
            # Permute inclusions as needed
            permuted_inclusions = np.array(inclusions)[self.perm]
            self.push_initial_design(permuted_inclusions)  # only pass the permuted inclusions

    def push_initial_design(self, permuted_inclusions: Collection[float]):
        """
        The inclusions are ALREADY permuted.
        We need to map the current permuted index i to the original index using self.perm.
        """
        events: list[tuple[float, str, int]] = []
        level: float = 0
        for i, p in enumerate(permuted_inclusions):
            original_index = int(self.perm[i])  # Map permuted index i -> original index
            next_level = level + p

...
class DublyBalancedKMeans:
    ...
    def fit(self, coords, probs):
        ...
        # Ensure no unit has sum(membership)==0
        missing = np.where(membership.sum(axis=1) == 0)[0]
        for i in missing:
            assignment, counts = np.unique(labels[expanded_idx == i], return_counts=True)
            if len(assignment) > 0:
                membership[i, assignment[np.argmax(counts)]] = 1.0

        # Show units with soft membership (optional, keep if helpful)
        ...
        # Build clusters using **original probs**
        clusters = []
        for j in range(self.k):
            ids = np.where(membership[:, j] > 0)[0]
            membership_weights = membership[ids, j]
            # Use the original probs as "mass" to split across clusters:
            cluster_probs = probs[ids] * membership_weights
            units = np.concatenate([
                ids.reshape(-1, 1),
                coords[ids],
                cluster_probs.reshape(-1, 1)
            ], axis=1)
            clusters.append(units)
        idx = np.where(membership.sum(axis=1)==0)[0]
        print("Unassigned units at indices:", idx)
        print(membership[idx])  # print those rows
        self.clusters = clusters

        # Prob check: should be exactly sum(probs)
        ...
        print(f"Difference: {(probs.sum() - total_prob_sum):.8f}")
